...
page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/CNA/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if exists_in_tree(lm, repo_name):
            print 'CNA/%s already exists' % (repo_name)
            continue

        print 'Adding dependency: CNA/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "CNA/%s" % repo_name, "revision": "jellybean" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/cna.dependencies'
    syncable_repos = []
    ...
    if os.path.exists(dependencies_path):
        ...
        for dependency in dependencies:
            if not is_in_manifest("CNA/%s" % dependency['repository']):
